...
precipitation_daily_long <- pivot_longer(precipitation_daily_df, 
                                         cols = 3:2193, names_to = "Date", values_to = "Precipitation") %>%
  select(-ID) %>%
  mutate(Date = as.Date(Date)) %>%
...
daily_files <- list.files("data/daily_data/2023/", full.names = T)
files_to_read <- list.files("data/daily_data/2023/")
sp_read_order <- stringr::str_remove_all(files_to_read, c("daily_max_|.csv"))
...
for(x in names(species_list)) {
  records_validated[[x]] <- left_join(site_records_combined[[x]], validated_calls[[x]]) %>%
...
site_data[[i]] <- joined_data[[i]] %>%
  ...
  select(Site_f, Loc, Cluster, Year) %>%
  distinct() %>%
  arrange(Site_f)
...
fit <- cont_model_ms_beta$sample(data = stan_data,
                         ...
                         save_warmup = FALSE,
                         iter_sampling = ni,
                         iter_warmup = nw, adapt_delta = 0.90, refresh = 10)
