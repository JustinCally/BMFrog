...
cont_model_ms_norm <- cmdstan_model("stan/continuous_model_ms_my_norm3.stan")
```
...
```{r readinmods}
fit_norm <- readRDS("outputs/fit_norm.rds")
```
...
score_preds <- fit_norm$draws("score_pred", format = "matrix")
scores_unlist <- unlist(scores)

ppc_plots <- bayesplot::ppc_stat(y = scores_unlist,
                                 yrep = score_preds,
                                 stat=mean) +
      ggplot2::ggtitle(label = "mean") +
      # ggplot2::scale_x_continuous(limits = c(qmin, qmax)) +
      ggplot2::theme_bw() +
      ggplot2::theme(legend.position = "top")

posterior_checks_multispecies(model = fit_norm, 
...
heatmap <- joined_data[[i]] %>% 
  ggplot() + 
  geom_tile(aes(x = Day, y = Site_Year, fill = Prob_f, colour = Prob_f)) + 
  geom_text(aes(x = 260, y = Site_Year, label = label), data = psi_summary_species, size = 4) +
...
```{r, fig.height = 8}
mu_summary <- fit_norm$draws("mu_pred") 
...
# beta_summaries 
beta_summaries <- fit_norm$summary("beta_psi")
...
beta_draws <- fit_norm$draws("beta_psi", format = "df")
...
# beta_summaries 
theta_summaries <- fit_norm$summary("beta_theta")
...
ab_joined_list <- list()
for(j in 1:length(species_list_cleaned)) {
theta_draws <- fit_norm$draws("beta_theta", format = "df")
...
zhat_draws <- fit_norm$draws("z_hat", format = "matrix") %>%
  as.data.frame() %>%
...
theta_hat_draws <- fit_norm$draws("theta_hat", format = "matrix") %>%
  as.data.frame() %>%
  pivot_longer(cols = everything()) %>%
  mutate(year = rep(sort(unique(site_metadata_sf$`Monitoring year`)), length.out = nrow(.)),
         species = rep(names(species_list_cleaned),
...
rel_ab_draws <- fit_norm$draws("rel_ab_mean", format = "matrix") %>%
  as.data.frame() %>%
  pivot_longer(cols = everything()) %>%
  mutate(year = rep(sort(unique(site_metadata_sf$`Monitoring year`)), length.out = nrow(.)),
         species = rep(names(species_list_cleaned), 
