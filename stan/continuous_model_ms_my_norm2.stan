...
for(i in 1:nsites) {
  ...
  mu[j,1,i] = mu_int[1,j] + eps_site[i,j];
  mu[j,2,i] = mu_int[2,j] + eps_site[i,j];
  psi[i,j] = inv_logit(psi_mm[j, i] * beta_psi[j] + eps_loc[loc_code[i,j], j] + eps_year[year_code[i,j], j]);
}
...
for(j in 1:nspec) {
  ...
  mu_int[1,j] ~ normal(-3,1);
  mu_int[2,j] ~ normal(3,1);
  sigma[,j] ~ normal(0,2);
...
for(i in 1:nsites) {
  ...
  if(start_idx_0[i,j] != 0) {
    if(any_seen[i,j] == 0) {
      target += log1m(psi[i,j]) + normal_lpdf(score[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[j, 1, i], sigma[1,j]);
    }
      target += log(psi[i,j]) + log1m(theta[j, start_idx_0[i,j]:end_idx_0[i,j]]) + normal_lpdf(score[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[j, 1, i], sigma[1,j]);
}
  if(start_idx_1[i,j] != 0) {
    target += log(psi[i,j]) + log(theta[j, start_idx_1[i,j]:end_idx_1[i,j]]) + normal_lpdf(score[start_idx_1[i,j]:end_idx_1[i,j],j] | mu[j, 2, i], sigma[2,j]);
}
  if(start_idx_2[i,j] != 0) {
        if(any_seen[i,j] == 0) {
    target += log1m(psi[i,j]) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[j, 1, i], sigma[1,j]);
        }
    target += log_sum_exp(log(psi[i,j]) + log_sum_exp(log1m(theta[j, start_idx_2[i,j]:end_idx_2[i,j]])) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[j, 1, i], sigma[1,j]),
    log(psi[i,j]) + log_sum_exp(log(theta[j, start_idx_2[i,j]:end_idx_2[i,j]])) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[j, 2, i], sigma[2,j]));
}
}
}
...
generated quantities {
    ...
    array[nspec, 2, nsites] real mu_pred;
...
for(j in 1:nspec) {
    ...
    for(n in 1:nsites) {
      mu_pred[j,1,n] = normal_rng(mu[j,1,n], sigma[1,j]);
      mu_pred[j,2,n] = normal_rng(mu[j,2,n], sigma[2,j]);
    ...
    for(k in 1:nfiles) {
      if(det_z[k,j] == 0) {
        score_pred[k,j] = normal_rng(mu_int[1,j], sigma[1,j]);
      } else if(det_z[k,j] == 1) {
        score_pred[k,j] = normal_rng(mu_int[2,j], sigma[2,j]);
      }
    }
}
for(j in 1:nspec) {
