...
transformed data {
  array[nfiles, nspec] real score_logit;
  score_logit = logit(score);
}
...
for(j in 1:nspec) {
  ...
  mu[1,j] = mu_int[1] + mu_sd[1] * mu_raw[1,j];
  mu[2,j] = mu_int[2] + mu_sd[2] * mu_raw[2,j];
  // linear model for sigma
  sigma[1,j] = exp(sigma_int[1] + sigma_sd[1] * sigma_raw[1,j]);
  sigma[2,j] = exp(sigma_int[2] + sigma_sd[2] * sigma_raw[2,j]);
  // locations
  for(l in 1:nloc) {
...
for(i in 1:nsites) {
  ...
  if(start_idx_0[i,j] != 0) {
    if(any_seen[i,j] == 0) {
      target += log1m(psi[i,j]) + normal_lpdf(score_logit[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[1, j], sigma[1,j]);
    }
      target += log(psi[i,j]) + log1m(theta[j, start_idx_0[i,j]:end_idx_0[i,j]])
      + normal_lpdf(score_logit[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[1, j], sigma[1,j]);
}
  if(start_idx_1[i,j] != 0) {
    target += log(psi[i,j]) + log(theta[j, start_idx_1[i,j]:end_idx_1[i,j]])
    + normal_lpdf(score_logit[start_idx_1[i,j]:end_idx_1[i,j],j] | mu[2, j], sigma[2,j]);
}
  if(start_idx_2[i,j] != 0) {
        if(any_seen[i,j] == 0) {
    target += log1m(psi[i,j])
    + normal_lpdf(score_logit[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[1, j], sigma[1,j]);
        }
    ...
    + normal_lpdf(score_logit[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[2, j], sigma[2,j]));
}
}
}
