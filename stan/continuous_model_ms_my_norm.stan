...
for(i in 1:nsites) {
  ...
  if(start_idx_0[i,j] != 0) {
    if(any_seen[i,j] == 0) {
      target += log1m(psi[i,j]) + normal_lpdf(score[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[1, j], sigma[1,j]);
    }
      target += log(psi[i,j]) + log1m(theta[j, start_idx_0[i,j]:end_idx_0[i,j]]) + normal_lpdf(score[start_idx_0[i,j]:end_idx_0[i,j],j] | mu[1, j], sigma[1,j]);
}
  if(start_idx_1[i,j] != 0) {
    target += log(psi[i,j]) + log(theta[j, start_idx_1[i,j]:end_idx_1[i,j]]) + normal_lpdf(score[start_idx_1[i,j]:end_idx_1[i,j],j] | mu[2, j], sigma[2,j]);
}
  if(start_idx_2[i,j] != 0) {
        if(any_seen[i,j] == 0) {
    target += log1m(psi[i,j]) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[1, j], sigma[1,j]);
        }
    target += log_sum_exp(log(psi[i,j]) + log_sum_exp(log1m(theta[j, start_idx_2[i,j]:end_idx_2[i,j]])) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[1, j], sigma[1,j]),
    log(psi[i,j]) + log_sum_exp(log(theta[j, start_idx_2[i,j]:end_idx_2[i,j]])) + normal_lpdf(score[start_idx_2[i,j]:end_idx_2[i,j],j] | mu[2,j], sigma[2,j]));
}
}
}
...
for(j in 1:nspec) {
    ...
    mu_pred[,j] = normal_rng(mu[,j], sigma[,j]);
    for(i in 1:365) {
    ...
    for(k in 1:nfiles) {
      if(det_z[k,j] == 0) {
        score_pred[k,j] = normal_rng(mu[1,j], sigma[1,j]);
      } else if(det_z[k,j] == 1) {
        score_pred[k,j] = normal_rng(mu[2,j], sigma[2,j]);
      }
    }
}
for(j in 1:nspec) {
